...
router.post('/orders', async (req, res) => {
  try {
    ...
    // keep only the need fields from items
    ...
    const { insertedId } = await db.collection('orders').insertOne(orderDoc); // Insert into 'orders' collection

    res.status(201).json({ 
      message: 'Order created successfully',
      _id: insertedId, 
      ...orderDoc }); // Respond with created order
...
router.get('/orders', async (_req, res) => {
  ...
  .find()
  .sort({ createdAt: -1 })
  .toArray(); // Fetch all orders sorted by creation date
res.json(orders);
});
